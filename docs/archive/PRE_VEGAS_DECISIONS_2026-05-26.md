...
1. **`inferno_paper_mark_to_market.py`** — refreshes Schwab quotes on every open paper ticket each cycle, computes current spread mid, and writes `reports/paper_mark_to_market_latest.txt` without mutating the ledger. Run it with `./run_inferno_paper_mark_to_market.sh`. Unblocks every price-triggered playbook rule.
